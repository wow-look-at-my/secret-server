...
- **Management:** create, list (by name + prefix, showing the granted secrets and bound policy), edit (change the secrets and/or policy), and revoke on the **Machine Tokens** admin page, or via `GET/POST/PUT/DELETE /admin/v1/machine-tokens` (create/update take `{name, policy_id?, node_ids?}`; create returns `{id, token}` — the only time the token is exposed).
