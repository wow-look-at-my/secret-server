...
Policies are pure pattern-match rules that you **attach** to one or more nodes. Each policy has a **mode** that determines how authorization is performed:

### Pattern mode (default)

The traditional mode where the server checks repo/ref/actor patterns. The policy specifies:
...
- **Repository patterns** — zero or more SQLite GLOB patterns matching repository names (e.g. `myorg/*`, `myorg/api-*`). A request matches if the repository matches **any** of the listed globs. Leaving this empty is allowed: the policy then has no repository patterns and matches nothing (see below), so you can create a placeholder policy now and add patterns later without granting any access in the meantime.
- **Ref patterns** — zero or more SQLite GLOB patterns matching git refs (e.g. `refs/heads/main`, `refs/tags/v*`). Leave blank to match nothing (no implicit wildcard) — write `*` explicitly to match any ref.
- **Actor patterns** — zero or more SQLite GLOB patterns matching the GitHub username that triggered the workflow (e.g. `deploy-*`). Leave blank to match nothing (no implicit wildcard) — write `*` explicitly to match any actor.
...
3. Finds every policy whose pattern rows all match the repository, ref, and resolved actor login or `id:<numeric ID>` (single SQL query via `GLOB` joins). For github-environment mode policies, only repo patterns and the OIDC `environment` claim are matched.
