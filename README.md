...
2. Finds every policy whose pattern rows all match the token's repository, ref, and actor claims (single SQL query via `GLOB` joins). For github-environment mode policies, only repo patterns and the OIDC `environment` claim are matched.
