...
// it is shown to the operator exactly once, at creation.
...
// CreateMachineToken mints a token bound to the given environment and stores
// only its hash. The returned plaintext token is the only time it is ever
// available — the caller must surface it to the operator and then discard it.
func (d *DB) CreateMachineToken(name, environmentID string) (token string, rec *MachineToken, err error) {
