...
func (d *DB) CreatePolicy(name, mode string, repoPatterns, refPatterns, actorPatterns []string, githubEnvironment string) (*Policy, error) {
	if name == "" {
	...
	if mode != PolicyModePattern && mode != PolicyModeGitHubEnvironment {
		return nil, fmt.Errorf("invalid mode %q: must be %q or %q", mode, PolicyModePattern, PolicyModeGitHubEnvironment)
	}
	if mode == PolicyModeGitHubEnvironment && githubEnvironment == "" {
		return nil, fmt.Errorf("github_environment is required when mode is %q", PolicyModeGitHubEnvironment)
	}
...
//
// This uses a raw SQL query instead of d.q.GetPolicy because the sqlc-generated
// query string for GetPolicy was not regenerated to include the mode and
// github_environment columns added by the GitHub Actions Environment feature.
// The source queries/policies.sql is correct; running sqlc generate would fix
// the generated code. Until then we query the 5 columns directly.
func (d *DB) GetPolicy(id string) (*Policy, error) {
	ctx := context.Background()
	var row sqlcdb.AccessPolicy
	err := d.db.QueryRowContext(ctx,
		`SELECT id, name, mode, github_environment, created_at FROM access_policies WHERE id = ?`,
		id,
	).Scan(&row.ID, &row.Name, &row.Mode, &row.GithubEnvironment, &row.CreatedAt)
...
func (d *DB) hydratePolicy(ctx context.Context, row sqlcdb.AccessPolicy) (*Policy, error) {
	...
	p := &Policy{ID: row.ID, Name: row.Name, Mode: row.Mode, GitHubEnvironment: row.GithubEnvironment, CreatedAt: row.CreatedAt}
...
// UpdatePolicy replaces a policy's name, mode, github_environment, and full
// pattern set in one tx. The old pattern rows are deleted and the new ones
// inserted.
func (d *DB) UpdatePolicy(id, name, mode string, repoPatterns, refPatterns, actorPatterns []string, githubEnvironment string) error {
	if name == "" {
	...
	if mode != PolicyModePattern && mode != PolicyModeGitHubEnvironment {
		return fmt.Errorf("invalid mode %q: must be %q or %q", mode, PolicyModePattern, PolicyModeGitHubEnvironment)
	}
	if mode == PolicyModeGitHubEnvironment && githubEnvironment == "" {
		return fmt.Errorf("github_environment is required when mode is %q", PolicyModeGitHubEnvironment)
	}
	if err := ValidatePatterns(repoPatterns); err != nil {
		return err
	}
	...
	res, err := q.UpdatePolicyMode(ctx, sqlcdb.UpdatePolicyModeParams{
		Name:              name,
		Mode:              mode,
		GithubEnvironment: githubEnvironment,
		ID:                id,
...
FROM access_policies p
JOIN policy_patterns pr ON pr.policy_id = p.id AND pr.kind = 'repository'
WHERE p.mode = 'github-environment'
  AND ? GLOB replace(pr.pattern, '[', '[[]')
  AND p.github_environment = ?
  AND ? != ''
`
...
//
// For github-environment mode policies, only the repository pattern and the
// OIDC environment claim are checked (exact equality on github_environment);
// ref/actor filtering is delegated to GitHub's environment protection rules.
//
