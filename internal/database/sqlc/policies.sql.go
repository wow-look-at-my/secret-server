...
func (q *Queries) GetPolicy(ctx context.Context, id string) (AccessPolicy, error) {
	...
	var i AccessPolicy
	err := row.Scan(&i.ID, &i.Name, &i.Mode, &i.GithubEnvironment, &i.CreatedAt)
	return i, err
}
...
func (q *Queries) ListPolicies(ctx context.Context) ([]AccessPolicy, error) {
	...
	for rows.Next() {
		var i AccessPolicy
		if err := rows.Scan(&i.ID, &i.Name, &i.Mode, &i.GithubEnvironment, &i.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const updatePolicyMode = `-- name: UpdatePolicyMode :execresult
UPDATE access_policies SET name = ?, mode = ?, github_environment = ? WHERE id = ?
`

type UpdatePolicyModeParams struct {
	Name              string
	Mode              string
	GithubEnvironment string
	ID                string
...
func (q *Queries) UpdatePolicyMode(ctx context.Context, arg UpdatePolicyModeParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updatePolicyMode, arg.Name, arg.Mode, arg.GithubEnvironment, arg.ID)
}
