...
func (h *AdminHandler) createPolicy(w http.ResponseWriter, r *http.Request) {
	...
	policy, err := h.db.CreatePolicy(req.Name, req.Mode, req.RepositoryPatterns, req.RefPatterns, req.ActorPatterns, req.GitHubEnvironment)
	if err != nil {
...
func (h *AdminHandler) updatePolicy(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.UpdatePolicy(id, req.Name, req.Mode, req.RepositoryPatterns, req.RefPatterns, req.ActorPatterns, req.GitHubEnvironment); err != nil {
		if errors.Is(err, database.ErrNotFound) {
			http.Error(w, `{"error":"policy not found"}`, http.StatusNotFound)
			return
		}
