...
func attachPolicyTo(t *testing.T, env *testEnv, secretName, value string, parentID *string,
	policyName, mode string, repo, ref, actor []string, githubEnvironment string) (secretID, policyID string) {
	t.Helper()
	s, err := env.db.CreateSecret(parentID, secretName, value)
	require.Nil(t, err)
	p, err := env.db.CreatePolicy(policyName, mode, repo, ref, actor, githubEnvironment)
	require.Nil(t, err)
...
func TestPublicFetchSecretsWithPolicy(t *testing.T) {
	...
	attachPolicyTo(t, env, "DB_URL", "postgres://localhost", nil,
		"allow", "pattern", []string{"myorg/*"}, []string{"*"}, []string{"*"}, "")
...
func TestPublicFetchSecretsNoMatchingPolicy(t *testing.T) {
	...
	attachPolicyTo(t, env, "KEY", "val", nil,
		"other", "pattern", []string{"otherorg/*"}, []string{"*"}, []string{"*"}, "")
...
func TestPublicFetchSecretsEmptyPolicyGrantsNoAccess(t *testing.T) {
	...
	attachPolicyTo(t, env, "KEY", "val", nil,
		"empty", "pattern", nil, []string{"*"}, []string{"*"}, "")
...
func TestPublicFetchSecretsMultipleSecretsSamePolicy(t *testing.T) {
	...
	require.Nil(t, err)
	p, err := env.db.CreatePolicy("p", "pattern", []string{"myorg/*"}, []string{"*"}, []string{"*"}, "")
	require.Nil(t, err)
...
func TestPublicFetchSecretsActorPatternMatch(t *testing.T) {
	env := setup(t)
	attachPolicyTo(t, env, "DB_URL", "postgres://localhost", nil,
		"allow-deployer", "pattern", []string{"myorg/*"}, []string{"*"}, []string{"deploy-*"}, "")
...
func TestPublicFetchSecretsGitHubEnvMode(t *testing.T) {
	...
	attachPolicyTo(t, env, "API_KEY", "sk-live-123", nil,
		"ghenv", "github-environment", []string{"myorg/*"}, nil, nil, "production")
