...
func TestAdminAttachDetachPolicy(t *testing.T) {
	...
	require.Nil(t, err)
	p, err := env.db.CreatePolicy("p", "pattern", []string{"org/*"}, []string{"*"}, []string{"*"}, "")
	require.Nil(t, err)
...
func TestAdminAddRemovePrecedence(t *testing.T) {
	...
	a, _ := env.db.CreatePolicy("a", "pattern", []string{"*"}, []string{"*"}, []string{"*"}, "")
	b, _ := env.db.CreatePolicy("b", "pattern", []string{"*"}, []string{"*"}, []string{"*"}, "")
...
func TestAdminListNodePolicies(t *testing.T) {
	env := setup(t)
	g, _ := env.db.CreateGroup(nil, "g")
	p, _ := env.db.CreatePolicy("p", "pattern", []string{"*"}, []string{"*"}, []string{"*"}, "")
...
func TestAdminCreatePolicyGitHubEnvMode(t *testing.T) {
	...
	body := `{"name":"ghenv","mode":"github-environment","repository_patterns":["org/*"],"github_environment":"production"}`
	req := jsonReq("POST", "/admin/v1/policies", body)
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, req)
	require.Equal(t, http.StatusCreated, rr.Code)
...
func TestAdminCreatePolicyGitHubEnvModeMissingEnv(t *testing.T) {
	...
	body := `{"name":"ghenv","mode":"github-environment","repository_patterns":["org/*"]}`
	req := jsonReq("POST", "/admin/v1/policies", body)
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, req)
	assert.Equal(t, http.StatusBadRequest, rr.Code)
...
func TestAdminCreatePolicyInvalidMode(t *testing.T) {
	...
	body := `{"name":"bad","mode":"invalid","repository_patterns":["org/*"]}`
	req := jsonReq("POST", "/admin/v1/policies", body)
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, req)
	assert.Equal(t, http.StatusBadRequest, rr.Code)
