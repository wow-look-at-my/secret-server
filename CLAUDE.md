...
- **Policy-based access**: Each policy has a `mode` field: `pattern` (default) or `github-environment`. In **pattern** mode, glob patterns for repository, ref, and actor are matched (AND across fields, OR within). In **github-environment** mode, only repo patterns and the OIDC `environment` claim are checked — ref/actor filtering is delegated to GitHub's environment protection rules. Patterns are stored as JSON arrays in `repository_patterns` / `ref_patterns` / `actor_patterns` TEXT columns. An empty list acts as a wildcard. The `github_environment` TEXT column stores the expected environment name for github-environment mode. Legacy columns are auto-migrated on upgrade.
