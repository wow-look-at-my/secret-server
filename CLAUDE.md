...
- **Policy modes**: Each policy has a `mode` field: `pattern` (default) or `github-environment`. In **pattern** mode, glob patterns for repository, ref, and actor are matched (AND across fields, OR within). In **github-environment** mode, only repo patterns and the OIDC `environment` claim are checked — ref/actor filtering is delegated to GitHub's environment protection rules. The `github_environment` TEXT column stores the expected environment name for github-environment mode. `MatchingPolicyIDs` accepts an `oidcEnvironment` parameter and uses a UNION of pattern-mode and github-environment-mode queries. On upgrade, `mode` and `github_environment` columns are auto-added via `migratePolicyMode`.
- **Machine tokens**: bearer credentials (`sst_<random>`, SHA-256–hashed at rest) for clients that can't present a GitHub OIDC JWT (e.g. webhook-runner hooks). A token grants secrets **two ways, either or both**: by attaching **directly to secret-tree nodes** (`machine_token_nodes` junction) and/or via an **optional bound policy** (the nullable `machine_tokens.policy_id`). Separately, the default-off `can_attest_github_pushes` capability permits the Agent Host control plane to call `HEAD/POST /github/v1/push-provenance`; secret grants never imply that authority, and such a token must never enter a job or agent container. `fetchSecretsMachine` (public.go) vends `AuthorizedSecretsForToken`, the same downward-walk CTE as the OIDC path but seeded from **both** the token's direct attachments and its policy's attached nodes (attaching/binding a **group** grants its whole subtree). Direct attachment is the no-busywork default; a policy is for a reusable grant shared across tokens. Both credential types share `GET /github/v1/secrets` (the `database.MachineTokenPrefix` on the bearer token selects the path — no extra CF bypass). Tables `machine_tokens` (nullable `policy_id` FK, `ON DELETE SET NULL` — deleting a policy just unbinds it) + `machine_token_nodes` (FKs `ON DELETE CASCADE`); `CreateMachineToken`/`UpdateMachineToken` set the policy + replace the node attachments within one transaction (an unknown node **or** policy ID is a clean `ErrNotFound` rollback). Admin REST CRUD at `/admin/v1/machine-tokens` (create/list/update/delete, plus `POST /admin/v1/machine-tokens/{id}/regenerate` — `RegenerateMachineToken` mints a fresh `sst_` value in place, updating only the stored hash+prefix so name/policy/attachments survive while the old value dies instantly; returns `{id, token}` like create, audited as `machine_token.regenerate`; create/update take `{name, policy_id?, node_ids?, can_attest_github_pushes?}`) + the **Machine Tokens** UI page (create/edit with a secret-tree checkbox picker + an optional policy dropdown, revoke, regenerate — the new value renders on the same show-once page as creation); the UI exposes the attestation capability with a high-trust warning. Ordinary grant updates preserve it; admin updates change grants and capability atomically. Every vend audited as `secret.access.granted` / actor `machine_token`. Plaintext shown once, at creation or regeneration. On upgrade, the original `NOT NULL` `policy_id` is relaxed to nullable (`migrateMachineTokensPolicyOptional`: a one-time, secret-data-safe rebuild gated on `columnNotNull`, preserving existing bindings); existing tokens migrate with attestation disabled.
